...
console.log(totaltareas)
  ...
  const handleEstadoChange = (id, nuevoEstado) => {
    ...
    onUpdate(nuevasTareas); // Notifica al componente padre sobre la actualización
  };
      ...
      {totaltareas.map((tarea) => (
        <Card key={tarea.NUMEXPEDTAREAID} className='rounded-lg'>
          <div className={`p-6 justify-between flex w-full items-center relative`}>
            ...
            <div className='flex flex-col gap-2'>
              <span
                className={`p-2 rounded-lg uppercase font-bold text-sm text-center ${getEstadoClassName(tarea.VCHESTADO
                )}`}
              >
                {tarea.VCHESTADO}
              </span>
